...
class DMN_qa:
    ...
    def __init__(self, babi_train_raw, babi_test_raw, word2vec, word_vector_size, 
        ...
        self.vocab_size = 5 # number of answer choices
        ...
        self.c_vecs = []
        for choice in [self.ca_var, self.cb_var, self.cc_var, self.cd_var,self.ce_var]:
        ...
        self.c_vecs = T.stack(self.c_vecs).transpose((1, 0)) # (dim, 5)
        self.inp_c = T.stack([self.inp_c] * 5).transpose((1, 2, 0)) # (fact_cnt, dim, 5)
        self.q_q = T.stack([self.q_q] * 5).transpose((1, 0)) # (dim, 5)
        ...
        self.W_a = theano.shared(lasagne.init.Normal(0.1).sample((self.vocab_size, 5 * self.dim)), borrow=True)
        ...
        if self.mode == 'train':
            ...
            self.train_fn = theano.function(inputs=[self.inp_var, self.q_var, self.ans_var,
                                                    self.ca_var, self.cb_var, self.cc_var, self.cd_var,self.ce_var,
                                                    self.input_mask_var], 
    ...
    def new_attention_step(self, ct, prev_g, mem, q_q, c_vecs):
        cWq = T.dot(T.ones((1, 5), dtype=floatX), T.dot(T.dot(ct.T, self.W_b), q_q) * T.eye(n=5, m=5, dtype=floatX))
        cWm = T.dot(T.ones((1, 5), dtype=floatX), T.dot(T.dot(ct.T, self.W_b), mem) * T.eye(n=5, m=5, dtype=floatX))
        cWc_vecs = T.dot(T.ones((1, 5), dtype=floatX), T.dot(T.dot(ct.T, self.W_b), c_vecs) * T.eye(n=5, m=5, dtype=floatX))
    ...
    def _process_input(self, data_raw):
        ...
        for x in data_raw:
            ...
            pe = self._find_first(inp, 'e>')
            index_choice = [pa,pb,pc,pd,pe]
            choices_5 = []
            prev = None
            for i in index_choice:
                if prev != None:
                    if prev==-1:
                        choices_5.append(['none'])
                    elif i!=-1:
                        choices_5.append(inp[prev+1:i])
                    else:
                        choices_5.append(inp[prev+1:])
                prev = i
            if prev==-1:
                choices_5.append(['none'])
            else:
    ...
    def step(self, batch_index, mode):
        ...
        if mode == 'train':
            ...
            gradient_value = self.get_gradient_fn(inp, q, ans, ca, cb, cc, cd, ce, input_mask)
        ...
        if skipped == 0:
            ret = theano_fn(inp, q, ans, ca, cb, cc, cd,ce, input_mask)
        else:
